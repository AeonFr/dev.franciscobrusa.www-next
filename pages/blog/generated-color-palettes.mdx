...
Given a fixed value for hue (0) and saturation (0%), we generate classes that apply different degrees of luminosity. Basically we go from `hsl(0, 0%, 0%)` to `hls(0, 0%, 100%)`, by steps of `10%`. Check out the demo below.
...
Luckily, smarter people have come up with solutions and alternatives to deal with color in code. We'll explore two of them, HSLuv and Leonardo Contrast Colors.
...
For some scenarios, it might make sense to sacrifice some uniformity in the contrast dimension in pro of having more saturated colors. This tradeoff might make sense for some cases and not for others, so it's impossible for a fully automated tool to make this kind of judgements.
...
If you're interested in start using HSLuv in your workflow, you can start right away with the help of this tools:
...
HSLuv is a great tool that generates fast computations with minimal bundle size.

When bundle size is not important, though, you might want to use a more advanced tool. We will check Leonardo Contrast Colors as an alternative.
...
<Headline tag="h2" style={{ marginTop: "4em", maxWidth: "13em" }}>
  ...
  <br />
  An advanced suite to generate color palettes
</Headline>

What I immediately loved about [Leonardo Contrast Colors](https://www.npmjs.com/package/@adobe/leonardo-contrast-colors) is that it's specifically designed to generate color palettes with WCAG compliant contrast ratios.

This tool has a hugh size footprint for your website (88kb, vs the 2kb that HSLuv bring), so it's probably not suited for client-side, runtime generation of color palettes. It's probably better to use it for the internal tooling that surrounds your design system.

For example, you could use Leonardo Contrast Colors (LCC for short) to generate palettes at build-time.

It's usage is a little bit more complex than HSLuv as well. To begin with, we need to identify the "steps" we want to 

<AdobeContrastColorPalette hue={0} />
<AdobeContrastColorPalette hue={60} />
<AdobeContrastColorPalette hue={180} />
<AdobeContrastColorPalette hue={230} />
<AdobeContrastColorPalette hue={270} />
