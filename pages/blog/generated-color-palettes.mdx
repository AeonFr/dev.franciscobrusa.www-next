...
Most monitors use the sRGB color spectrum, which basically mean they can represent a limited subset of the colors we can see in real life.
...
This is the result of going from `hsl(0, 0%, 0%)` to `hsl(0, 0%, 100%)`. Since the saturation is 0, this will get a palette of grays.
...
Given a fixed value for hue (0) and saturation (0%), we generated classes that apply different degrees of luminosity. Basically we went from `hsl(0, 0%, 0%)` to `hls(0, 0%, 100%)`, by steps of `10%`.
...
In particular, CIECAM02-UCS performs much better for this use cases (thanks [Nate Baldwin](https://twitter.com/natebaldwinart) for pointing this out).

Alternative tools like [Chroma.js](https://gka.github.io/chroma.js/) and [Leonardo Color](https://www.npmjs.com/package/@adobe/leonardo-contrast-colors) can be used to generate palettes.
...
Still, if you **don't need to generate palettes with code, I recomend you to check the [Leonardo Color web UI](https://leonardocolor.io/)**. It uses the CIECAM02 color space by default, which produces some nice gradients between colors.
